...
def getTweets():
	...
	tekst2 = re.sub(r'[\n]{1}',' ',tekst1)#Fjerner \n
	tekst2 = re.sub(r'(&amp;)','&',tekst1)#bytter &amp; med &
	tekst3 = re.sub(r'([(https){1}(http){1}]{4,5}:\/\/){1}[^\.]+[.]{1}[\w]{2,3}[\S]+',' ',tekst2)#Fjerner nettadresser
	tekst4 = re.sub(r'[\s]+',' ',tekst3)# Gj0r mellomrom til et.
	tekstSum = summarize(tekst4,3)
	tekstSum2 = ''
	for tekst in tekstSum:
		tekstSum2 += ' ' + tekst
	postURL = 'https://api.twitter.com/1.1/statuses/update.json?' + urllib.parse.urlencode({'status': tekstSum2})
	post(client.request(postURL,'POST'))
